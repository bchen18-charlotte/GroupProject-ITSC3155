...
def create(request: schema.ReviewCreate, db: Session = Depends(get_db), _=Depends(require_customer)):
...
def update(item_id: int, request: schema.ReviewUpdate, db: Session = Depends(get_db), _=Depends(require_customer)):

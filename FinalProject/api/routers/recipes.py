...
def create(request: schema.RecipeCreate, db: Session = Depends(get_db), _=Depends(require_staff)):
...
def update(item_id: int, request: schema.RecipeUpdate, db: Session = Depends(get_db), _=Depends(require_staff)):

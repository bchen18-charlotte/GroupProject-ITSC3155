...
def create(request: schema.CommentResponseCreate, db: Session = Depends(get_db), _=Depends(require_customer)):
...
def update(item_id: int, request: schema.CommentResponseUpdate, db: Session = Depends(get_db), _=Depends(require_customer)):

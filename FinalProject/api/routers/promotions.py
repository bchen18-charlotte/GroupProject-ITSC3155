...
def create(request: schema.PromotionCreate, db: Session = Depends(get_db), _=Depends(require_staff)):
...
def update(item_id: int, request: schema.PromotionUpdate, db: Session = Depends(get_db), _=Depends(require_staff)):

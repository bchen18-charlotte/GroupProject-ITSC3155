...
def create(request: schema.CustomerCreate, db: Session = Depends(get_db), _=Depends(require_staff)):
...
def update(item_id: int, request: schema.CustomerUpdate, db: Session = Depends(get_db), _=Depends(require_customer)):

...
def read_active(db: Session = Depends(get_db), _=Depends(require_staff)):
    return controller.read_active(db)

@router.get("/status/{order_status}", response_model=list[schema.Order])
def read_by_status(order_status: str, db: Session = Depends(get_db), _=Depends(require_staff)):
...
def read_by_customer(customer_id: int, db: Session = Depends(get_db), _=Depends(require_customer)):
...
def read_by_date_range(start: datetime, end: datetime, db: Session = Depends(get_db), _=Depends(require_staff)):
    return controller.read_by_date_range(db, start=start, end=end)

@router.get("/{item_id}", response_model=schema.Order)
def read_one(item_id: int, db: Session = Depends(get_db), _=Depends(require_customer)):
    return controller.read_one(db, item_id=item_id)

@router.put("/{item_id}", response_model=schema.Order)
def update(item_id: int, request: schema.OrderUpdate, db: Session = Depends(get_db), _=Depends(require_staff)):
